...
class ManiSkillAsync2VectorEnv(VectorEnv):
    """
    Gymnasium Vector Env implementation for ManiSkill environments running on the GPU for parallel simulation and optionally parallel rendering
    ...
    def __init__(
        ...
        super().__init__(
            num_envs,
            self.base_env.get_attr("single_observation_space")[0],
            self.base_env.get_attr("single_action_space")[0]
        )
        device = self.base_env.get_attr("device")
        assert all(i == device[0] for i in device)
        self.device_list = device

        self.returns = torch.zeros(self.num_envs, device=self.device_list[0])
        self.max_episode_steps = max_episode_steps
        spec = self.base_env.get_attr("spec")
        self.spec_list = spec
        if (
                self.max_episode_steps is None
                and self.spec_list[0].max_episode_steps is not None
        ):
            self.max_episode_steps = self.spec_list[0].max_episode_steps
        if self.max_episode_steps is None:
            # search wrappers to find where max episode steps may have been defined
            self.max_episode_steps = gym_utils.find_max_episode_steps_value(env)
    ...
    def reset(
        ...
        if "env_idx" in options:
            env_idx = options["env_idx"]
            mask = torch.zeros(self.num_envs, dtype=bool, device=self.device_list[0])
            mask[env_idx] = True
            self.returns[mask] = 0
        else:
            self.returns *= 0
    ...
    def step(
        ...
        obs, rew, terminations, truncations, infos = self._env.step(actions)
        self.returns += rew.squeeze()

        infos["episode"] = dict(r=self.returns)

        # fix issue with gymnasium replacing truncations with a single bool
        cur_elapsed_steps = infos['elapsed_steps'] if 'elapsed_steps' in infos else infos['final_info']['elapsed_steps']
        # print("step_cnt:", cur_elapsed_steps)
        if self.handle_truncations and self.max_episode_steps is not None:
            truncations: torch.Tensor = (
                    cur_elapsed_steps >= self.max_episode_steps
            )
        if isinstance(terminations, bool):
            terminations = torch.tensor([terminations], device=self.device)
        # if terminations.any():
        ...
        if self.ignore_terminations:
            terminations[:] = False
        dones = torch.logical_or(terminations.squeeze(), truncations.squeeze())
        infos[
            "real_next_obs"
        ] = obs  # not part of standard API but makes some RL code slightly less complicated
        if dones.any():
...
class ManiSkillAsyncVectorEnv(VectorEnv):
    """
    Gymnasium Vector Env implementation for ManiSkill environments running on the GPU for parallel simulation and optionally parallel rendering
    ...
    def __init__(
        ...
        self.returns = torch.zeros(self.num_envs, device=torch.device("cpu"))
        self.max_episode_steps = max_episode_steps
        # if (
        #     self.max_episode_steps is None
        #     and self.base_env.spec.max_episode_steps is not None
