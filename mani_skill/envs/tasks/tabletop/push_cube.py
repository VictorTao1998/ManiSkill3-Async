...
class PushCubeEnv(BaseEnv):
    ...
    def __init__(self, *args, robot_uids="panda_wriststereocam", robot_init_qpos_noise=0.02, **kwargs):
...
@register_env("PushCube-v3", max_episode_steps=50)
class PushCubeEnv(BaseEnv):
    """
    Task Description
    ----------------
    ...
    def __init__(self, *args, robot_uids="panda_wristactivestereocam", robot_init_qpos_noise=0.02, **kwargs):
        # specifying robot_uids="panda" as the default means gym.make("PushCube-v1") will default to using the panda arm.
        self.robot_init_qpos_noise = robot_init_qpos_noise
        print(kwargs)
        super().__init__(*args, robot_uids=robot_uids, **kwargs)
    ...
    #             obs = stereo_sensor_data_to_rgbd(obs, self._sensors, rgb=True, depth=True, segmentation=True)
    ...
    #             obs = stereo_sensor_data_to_rgbd(obs, self._sensors, rgb=True, depth=False, segmentation=True)
...
@register_env("PushCube-v3-sb3", max_episode_steps=50)
class PushCubeEnv(BaseEnv):
    """
    Task Description
    ----------------
    ...
    def get_obs(self, info: Dict = None):
        ...
        for _, v in obs.items():
            obs_out.update(v)
        del obs_out['hand_camera_right']
        obs_out.update(obs_out['hand_camera'])
        del obs_out['hand_camera']

        return obs_out


@register_env("PushCube-v4", max_episode_steps=50)
class PushCubeEnv(BaseEnv):
    """
    Task Description
    ----------------
...
@register_env("PushCube-v5", max_episode_steps=50)
class PushCubeEnv(BaseEnv):
    """
    Task Description
    ----------------
    ...
    def __init__(self, *args, robot_uids="xmate3_robotiq", robot_init_qpos_noise=0.02, **kwargs):
        # specifying robot_uids="panda" as the default means gym.make("PushCube-v1") will default to using the panda arm.
        self.robot_init_qpos_noise = robot_init_qpos_noise
        print(kwargs)
        super().__init__(*args, robot_uids=robot_uids, **kwargs)
