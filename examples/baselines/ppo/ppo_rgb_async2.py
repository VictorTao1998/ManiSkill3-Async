...
from mani_skill.utils.wrappers.flatten import FlattenActionSpaceWrapper, FlattenRGBDObservationWrapper, FlattenRGBDObservationAsync2Wrapper
from mani_skill.utils.wrappers.record import RecordEpisode
from mani_skill.utils.wrappers.recordasync import RecordEpisodeAsync
from mani_skill.vector.wrappers.gymnasium import ManiSkillVectorEnv, ManiSkillAsync2VectorEnv
...
class Args:
    ...
    save_train_video_freq: Optional[int] = 10
...
if __name__ == "__main__":
    ...
    envs = ManiSkillAsyncEnv(
        [lambda: gym.make(args.env_id, **env_kwargs)] * (args.num_envs if not args.evaluate else 1))
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None:
            ...
            envs = RecordEpisodeAsync(envs, output_dir=f"runs/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=30)
        eval_envs = RecordEpisode(eval_envs, output_dir=eval_output_dir, save_trajectory=args.evaluate, trajectory_name="trajectory", max_steps_per_video=args.num_eval_steps, video_fps=30)
    envs = ManiSkillAsync2VectorEnv(envs, args.num_envs, ignore_terminations=False, **env_kwargs)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_done = torch.logical_or(terminations.squeeze(), truncations.squeeze()).to(torch.float32)
